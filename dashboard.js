...
// ===== TEMPAT TIDUR =====
// PENTING (sesuai standar Depkes RI 2005): rumus BOR/AvLOS/TOI/BTO memakai
// KAPASITAS TETAP tempat tidur untuk satu periode pelaporan penuh, BUKAN sisa
// tempat tidur kosong pada saat halaman dibuka. Snapshot harian hanya dipakai
// sebagai info di Beranda (statTTKosong), dan TIDAK dipakai sebagai pembagi
// pada perhitungan indikator manapun.
//
// Kolom "Jumlah Tempat Tidur" di Sensus Otomatis dan Indikator Rawat Inap kini
// bisa diedit manual (tidak readonly lagi) dan otomatis diisi dengan kapasitas
// total dari Pengaturan — tapi kalau pengguna sudah mengisi angka sendiri
// (misalnya untuk periode lama dengan kapasitas berbeda), nilai itu TIDAK akan
// ditimpa otomatis.
...
function perbaruiInfoTempatTidurKosong() {
  ...
  // Isi kolom kapasitas dengan nilai dari Pengaturan HANYA kalau kolom itu masih
  // kosong, supaya tidak menimpa angka yang sudah diisi/diubah manual oleh pengguna
  // (misalnya untuk menghitung ulang periode lama dengan kapasitas TT berbeda).
  if (totalTT > 0) {
...
// ===================================================
// ===== PENGATURAN TEMPAT TIDUR RUMAH SAKIT (GLOBAL) =====
// ===================================================
...
// ===== HITUNG OTOMATIS JUMLAH HARI PERIODE DARI TANGGAL AWAL-AKHIR =====
// Sesuai standar Depkes RI, denominator BOR/TOI harus memakai jumlah hari
// kalender yang sebenarnya dalam periode tersebut (28/29/30/31/365/366),
// bukan angka bulat seperti 30 yang sering salah kalau diketik manual.
// Kolom tanggal ini opsional: kalau diisi, kolom "Periode (jumlah hari)"
// otomatis dihitung dan dikunci; kalau dikosongkan, tetap bisa diisi manual
// seperti biasa.
...
document.getElementById('hitungForm').addEventListener('submit', function (e) {
  ...
  // Simpan input terakhir (termasuk tempatTidur & lamaDirawat) supaya saat halaman
  // dibuka lagi, periode yang sama tetap memakai angka yang sama persis — bukan
  // angka yang berubah-ubah mengikuti kondisi hari ini.
...
// ===== RINGKASAN MORTALITAS OTOMATIS PER TAHUN (DARI DATA PASIEN) =====
// Mengelompokkan SEMUA pasien pulang yang sudah ada di "Data Pasien" berdasarkan
// tahun Tanggal Keluar-nya. Setiap tahun yang punya data pasien akan otomatis
// terisi di sini — tidak perlu input manual lagi, selama data pasien tahun itu
// (dengan Tanggal Masuk/Keluar yang benar) sudah dimasukkan di Langkah 1.
...
function renderBarisMortalitas() {
  ...
  const JUMLAH_BARIS_MANUAL_TAMBAHAN = 2; // baris kosong ekstra untuk isi data tahun lama yang belum tercatat di sistem
...
document.getElementById('pasienMasukForm').addEventListener('submit', function (e) {
 ...
 const isian = {
  ...
  if (indexEditPasien !== null) {
    ...
    // Sinkronkan segmen PERTAMA riwayat penempatan (data masuk awal) dengan hasil edit,
    // supaya tidak menyisakan ruangan/kelas/departemen/tanggal masuk versi LAMA yang
    // sudah dikoreksi di form ini. Segmen perpindahan berikutnya (kalau ada) tidak diubah.
...
// ===== PASIEN PINDAHAN INTERN RS (pindah ruangan/kelas/departemen selama masih dirawat) =====
// Setiap pasien punya riwayatPenempatan: daftar segmen {tanggal, ruangan, kelas, departemen}
// terurut menaik. Dipakai untuk menentukan ruangan/kelas/departemen pasien yang BENAR pada
// tanggal tertentu (bukan cuma nilai terakhir), supaya sensus harian & RL1 akurat kalau ada
// pasien pindah ruangan di tengah masa rawat.
...
document.getElementById('pindahRuanganForm').addEventListener('submit', function (e) {
  ...
  // Field ruangan/poli/departemen utama ikut diperbarui ke penempatan terbaru,
  // supaya tampilan tabel pasien aktif & form lain (yang belum sadar konsep riwayat) tetap benar.
...
function bukaFormPulang(index) {
  ...
  // Kalau pasien ini sudah pernah diproses pulang sebelumnya, isi form dengan data
  // yang sudah ada supaya bisa diedit (bukan mulai dari kosong lagi).
...
// Keadaan Saat Pulang tetap bisa diisi untuk semua kategori Cara Keluar (bukan cuma
// "Pulang"), sesuai Buku Register RI resmi yang mengizinkan kondisi sembuh/belum sembuh
// dicatat meski pasien keluar lewat rujuk/pindah RS/pulang paksa/lari. Tidak wajib diisi.
...
document.getElementById('pulangForm').addEventListener('submit', function (e) {
  ...
  // Terjemahkan cara keluar detail ke kategori hidup/mati (dipakai rumus BOR/GDR/NDR yang sudah ada)
  let keadaanKeluar;
...
// ===================================================
// ===== 5. LAPORAN (REKAPITULASI & 10 BESAR OTOMATIS) =====
// ===================================================
...
document.getElementById('btnRekapSensus').addEventListener('click', function () {
  ...
  // PENTING: hanya mensyaratkan Tanggal Masuk, BUKAN Tanggal Keluar — supaya pasien
  // yang masih aktif dirawat (belum pulang) tetap ikut dihitung sebagai bagian dari
  // sensus harian (mereka tetap berkontribusi ke Hari Perawatan selama masih dirawat).
  // Sebelumnya pasien aktif ini terlewat sama sekali dari Sensus Otomatis, sehingga
  // BOR/AvLOS/TOI bisa jauh lebih kecil dari kondisi sebenarnya.
  ...
  while (tglJalan <= tglBatas) {
    ...
    // Pasien pindahan ruangan/kelas/departemen intern RS pada tanggal ini (informasional,
    // sesuai kolom "Pasien Pindahan" di Sensus RI resmi — tidak mengubah total sisa pasien
    // karena pindahan tidak menambah/mengurangi pasien di rumah sakit secara keseluruhan).
  ...
  // Koreksi pasien rawat sehari (masuk & keluar di tanggal sama): metode sensus
  // harian di atas menghitung mereka 0 hari (karena "hadir" hanya dihitung kalau
  // menginap semalam), padahal konvensi statistik RS standar tetap menghitung
  // pasien seperti ini sebagai 1 hari perawatan.
  ...
  // Total Lama Dirawat (untuk AvLOS) = jumlah (Tanggal Keluar - Tanggal Masuk) khusus
  // pasien yang SUDAH PULANG di periode ini — berbeda dari Hari Perawatan (untuk
  // BOR/TOI) yang berasal dari sensus harian dan ikut menghitung pasien yang belum pulang.
  ...
  // Simpan periode sensus terakhir supaya terisi otomatis saat halaman dibuka lagi
  localStorage.setItem(KUNCI_LAST_SENSUS_PERIODE, JSON.stringify({ awal, akhir }));

  // Ringkasan mortalitas per tahun otomatis mengikuti data pasien terbaru (lihat renderBarisMortalitas)
  renderBarisMortalitas();
...
function hitungSensusDepartemen(semuaPasien, dept, awal, akhir) {
  ...
  // Dihitung dari SELURUH pasien (bukan cuma yang departemen-nya sekarang = dept), supaya
  // hari-hari saat pasien masih di departemen ini SEBELUM pindah tetap terhitung di sini
  // (pakai penempatan pada tanggal tsb, bukan departemen terakhir pasien).
  while (tglJalan <= tglBatas) {
    const tglStr = tglJalan.toISOString().slice(0, 10);
    let hadirHariIni = 0;
  ...
  // PENTING: pasienAwalPeriode dan pasienSisa harus memakai penempatan pasien PADA
  // TANGGAL awal/akhir periode (bukan departemen SAAT INI/terakhir), supaya tetap akurat
  // untuk pasien yang sempat pindah departemen sebelum atau sesudah periode ini.
  ...
  // "Masuk" = pasien BARU yang pertama kali masuk RS lewat departemen ini pada periode
  // ini (bukan pasien pindahan dari departemen lain — itu sudah tercermin di kolom
  // "Pindahan" pada Sensus Harian/Rekap Bulanan).
  ...
  // Pindahan Masuk/Keluar KHUSUS perpindahan ANTAR DEPARTEMEN (bukan sekadar pindah
  // ruangan/kelas dalam departemen yang sama), supaya RL1 per departemen konsisten
  // dengan kolom "Pindahan" yang sudah ada di Sensus Harian & Rekap Bulanan.
...
document.getElementById('btnHitungRL1').addEventListener('click', function () {
  ...
  // Ambil dari SELURUH riwayat penempatan (bukan cuma departemen saat ini/terakhir),
  // supaya departemen yang pernah disinggahi pasien sebelum pindah tetap muncul di
  // laporan RL1 ini dan kontribusi hari perawatannya tidak hilang dari total.
  const daftarDept = [...new Set(semua.flatMap(p => {
    const riwayat = (p.riwayatPenempatan && p.riwayatPenempatan.length) ? p.riwayatPenempatan : [{ departemen: p.departemen }];
    return riwayat.map(r => r.departemen);
  }).filter(Boolean))];
...
// ===================================================
// ===== RENDER & PEMULIHAN DATA SAAT HALAMAN DIBUKA =====
// ===================================================
...
// Pulihkan input Indikator Rawat Inap terakhir supaya tidak perlu input ulang.
// Kini "tempatTidur" IKUT dipulihkan (tidak lagi dikecualikan), karena field ini
// sekarang berisi KAPASITAS TETAP untuk periode yang dihitung — bukan snapshot
// yang berubah-ubah — jadi aman dan justru benar untuk dipulihkan apa adanya.
(function muatIndikatorTersimpan() {
  ...
  try {
    ...
    // biarkan diam jika requestSubmit tidak didukung browser lama; data tetap sudah terisi
  }
})();